...
for (const product of products) {
  ...
  const intelligence = hasDetailedProfile ? `<section class="intel" aria-label="${sourceChecked ? 'Source-checked perfume profile' : 'Scentory supplied perfume profile'}"><span class="verified">Scentory supplied profile</span><h2>Perfume profile</h2><dl>${factRow('Scent character', (profile.character || []).map(titleCase))}${factRow('Notes', notes.listed)}${factRow('Opening notes', notes.top)}${factRow('Heart notes', notes.heart)}${factRow('Base notes', notes.base)}${factRow('Expected longevity', [`${details.performance || 'Skin, weather, batch and atomizer can change real-world performance.'}`])}${factRow('Best places to wear', (profile.occasions || []).map(value => occasionLabels[value] || titleCase(value)))}${factRow('Best conditions', (profile.climates || []).map(value => climateLabels[value] || titleCase(value)))}${details.comparison ? factRow('Compared with', [details.comparison]) : ''}</dl>${(details.sources || []).filter(source => /^https:\/\//.test(source.url || '')).map(source => `<p><a href="${esc(source.url)}" target="_blank" rel="noopener noreferrer">${esc(source.title || 'Product source')}</a></p>`).join('')}${details.sourceNote ? `<p>${esc(details.sourceNote)}</p>` : ''}<p>Performance is guidance, not a guarantee. Skin chemistry, heat, humidity and spray count can change the result.</p></section>` : `<section class="intel pending"><span class="reviewing">Profile review in progress</span><h2>Detailed profile</h2><p>Scentory is rechecking the exact note pyramid and performance sources for this perfume. Until that review is complete, detailed note claims are intentionally not shown. Please sample 5 ML first.</p></section>`;
  const purchase = `<section class="purchase" aria-label="Choose your decant"><p class="eyebrow">YOUR DECANT</p><h2>Choose a size</h2><div class="size-options">${Object.entries(product.sizes).map(([ml,item]) => `<label class="size-option"><input type="radio" name="size" value="${esc(ml)}" ${available && item.available ? '' : 'disabled'}><span><b>${esc(sizeLabel(ml))}${item.premium ? ' · Premium' : ''}</b><strong>৳${Number(item.price).toLocaleString('en-BD')}</strong><small>${product.status === 'upcoming' ? 'Upcoming' : available && item.available ? 'Available' : 'Out of stock'}</small></span></label>`).join('')}</div><div class="buy-row"><label for="quantity">Quantity</label><input id="quantity" type="number" min="1" max="20" value="1" inputmode="numeric"><button id="add-cart" type="button" ${available ? '' : 'disabled'}>Add to Cart</button></div><p id="cart-feedback" role="status" aria-live="polite"></p><a class="checkout-link" href="../#myOrder">View cart &amp; checkout →</a><p class="delivery-note">Delivery: ৳80 inside Dhaka · ৳130 outside Dhaka</p></section>`;
...
<link rel="stylesheet" href="../product-page-v3063.css"><link rel="icon" href="../favicon.svg" type="image/svg+xml"><script type="application/ld+json">${JSON.stringify(schema).replace(/</g, '\\u003c')}</script>
...
<body><header class="top"><a href="../" aria-label="Scentory home"><img src="../logo-horizontal-white.png" alt="Scentory"></a><nav aria-label="Main navigation"><a href="../#collection">Shop</a><a href="../#hotArrivals">New arrivals</a><a href="../#myOrder">Cart <span id="page-cart-count"></span></a></nav></header><div class="breadcrumb"><a href="../#collection">← Back to shop</a><button id="share-product" type="button">Share perfume ↗</button></div><main class="wrap"><div class="photo"><img src="../${esc(product.image)}" alt="${esc(product.name)} perfume decant" width="800" height="800" fetchpriority="high"></div><article><span class="status">${available ? 'Available at Scentory' : product.status === 'upcoming' ? 'Upcoming' : 'Currently out of stock'}</span><h1>${esc(product.name)} Decant</h1><p>${esc(description)}</p><div class="chips">${chips}</div>${purchase}${intelligence}<p class="small">Prices are in BDT and may change. Scentory prepares decants from original full-size perfume bottles and is not affiliated with the perfume brand shown.</p></article></main><footer class="page-footer">SCENTORY <span>Discover your next signature scent.</span><a href="../#collection">Explore 120+ perfumes →</a></footer><script id="product-data" type="application/json">${JSON.stringify(product).replace(/</g, '\\u003c')}</script><script src="../product-page-v3063.js" defer></script></body></html>`;
  if (/-poster-v\d+\.(jpg|png)$/.test(product.image)) {
...
const sitemapUrls = [
  ...
  ...products.map(product => `  <url><loc>${absolute(`perfume/${product.id}.html`)}</loc><lastmod>2026-09-11</lastmod><changefreq>weekly</changefreq><priority>0.7</priority></url>`)
];
